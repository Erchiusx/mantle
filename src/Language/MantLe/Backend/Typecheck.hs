...
type'check (Val'Var a) = do
  ...
  let def'type = datatype <$> find (\Declare{name = a'}->a' == a) declares
  case def'type of
    Nothing -> lift $ Left $ "cannot find type for var " <> show a <> "\n"
    Just t -> return t
...
apply'type :: Type'Expr -> Type'Expr -> Either String Type'Expr
